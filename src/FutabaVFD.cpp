...
//  Non-blocking scroll
// ---------------------------------------------------------------------------
void FutabaVFD::printScroll(const char* s, uint16_t stepMs, bool loop_, uint16_t holdMs) {
  if (!_initialised || s == nullptr) return;
  _vMode = V_IDLE;
  ...
  // Start in hold phase if holdMs > 0, so the reader sees the beginning
  // of the message before it starts scrolling.
...
void FutabaVFD::printScroll(const String& s, uint16_t stepMs, bool loop_, uint16_t holdMs) {
  printScroll(s.c_str(), stepMs, loop_, holdMs);
}
...
void FutabaVFD::advanceScroll() {
  ...
  if ((size_t)_scrollPos + _digits > _scrollLen + _digits) {
    if (_scrollLoop) {
      _scrollPos = 0;
      // Re-enter hold phase on loop restart so the reader can catch
      // the beginning of the message again.
      if (_scrollHoldMs > 0) {
        _scrollHolding = true;
        _scrollLastMs  = millis();
      }
